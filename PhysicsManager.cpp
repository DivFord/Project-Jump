...
PhysicsManager::PhysicsManager() : AsyncTask(std::string("PhysicsManager"))
{
	bulletWorld = new BulletWorld();
	bulletWorld->set_gravity(0, 0, -9.81);
}

AsyncTask::DoneStatus PhysicsManager::do_task()
{
...
void PhysicsManager::attach(TypedObject* object)
{
	bulletWorld->attach(object);
}

LVector3 PhysicsManager::get_gravity()
{
	return bulletWorld->get_gravity();
}

BulletClosestHitSweepResult PhysicsManager::sweep_test(BulletShape* shape, const TransformState& from_ts, const TransformState& to_ts, const CollideMask& mask)
{
	return bulletWorld->sweep_test_closest(shape, from_ts, to_ts, mask);
}

void PhysicsManager::create_debug(NodePath render)
{
